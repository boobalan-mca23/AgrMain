...
const CustomerOrders = () => {
 ...
 axios.delete(`${BACKEND_SERVER_URL}/api/customerOrder/image/${imgId}`)
 )
 );
 }
 ...
 if (item.images?.length > 0) item.images.forEach((f) => formData.append("images", f));
 ...
 await axios.post(`${BACKEND_SERVER_URL}/api/customerOrder/create`, formData, {
 headers: { "Content-Type": "multipart/form-data" },
 });
 ...
 */}
 ...
 <IconButton color="success" onClick={() => handleAddItemToGroup(order)}>
 <AddCircleOutline />
 </IconButton>
 </Tooltip>
 ...
 <TableCell align="center">{item.dueDate || "N/A"}</TableCell>
 <TableCell align="center">{item.workerName || "N/A"}</TableCell>
 <TableCell align="center">
 <StatusChip label={item.status} status={item.status} icon={getStatusIcon(item.status)} />
 </TableCell>
 <TableCell align="center">
 <Tooltip title="Edit this item">
 <IconButton color="info" onClick={() => handleEditRecord(order, item)}>
 <Edit />
 </IconButton>
 </Tooltip>

 <Tooltip title="Delete this item">
 <IconButton color="error" onClick={() => handleDeleteOrderItem(item.id)}>
 <Delete />
 </IconButton>
 </Tooltip>
 </TableCell>
 ...
 >
 <Typography variant="h6">{editingOrder ? "Edit Order" : "Create New Order"}</Typography>
 ...
 <Button variant="outlined" component="span" startIcon={<ImageIcon />} fullWidth sx={{ height: "40px" }}>
 Upload Image
 </Button>
 </label>
 ...
 <Box key={`existing-${pIdx}`} sx={{ position: "relative" }}>
 <ImagePreview onClick={() => handleOpenImageModal(img.url)}>
 ...
 const removed = updatedItems[index].existingImages.splice(pIdx, 1)[0];
 // remove the preview url
 updatedItems[index].imagePreviews = updatedItems[index].imagePreviews.filter((url) => url !== img.url);
 ...
 {item.imagePreviews?.slice(item.existingImages?.length).map((preview, pIdx) => (
 ...
 onClick={() => {
 const updatedItems = [...items];
 const relativeIdx = pIdx + (updatedItems[index].existingImages?.length || 0);

 // revoke the blob URL to free memory
 const previewUrl = updatedItems[index].imagePreviews[relativeIdx];
 ...
 updatedItems[index].imagePreviews.splice(relativeIdx, 1);
 ...
 <IconButton onClick={() => handleRemoveItem(index)} color="error" sx={{ alignSelf: "center" }}>
 <Close fontSize="small" />
 </IconButton>
 </Tooltip>
 )}
 ...
 <Button variant="outlined" startIcon={<Add />} onClick={handleAddItem} sx={{ mt: 1 }}>
 Add Another Item
 </Button>
 </DialogContent>
 ...
 variant="contained"
 onClick={handleSave}
 disabled={!items.every((item) => item.itemName && item.description && item.weight && item.dueDate && item.workerName)}
 sx={{
 bgcolor: "primary.main",
 "&:hover": { bgcolor: "primary.dark" },
 px: 3,
